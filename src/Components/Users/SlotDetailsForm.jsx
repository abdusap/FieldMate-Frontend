...
const navigate=useNavigate()
  ...
  return (
   ...
   className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 focus:outline-none block w-full md:w-[22rem] p-2.5">
            ...
            <input required onChange={(e)=>{setSelectedDate(e.target.value);setLoading(false)}}   type="date" min={minDate} id="first_name" className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 focus:outline-none block w-full md:w-[22rem] p-2.5 mb-4"  />
        </div>
...
{loading ?
  ...
  <div key={i}>
    ...
    <div role="status" className='flex justify-center items-center w-full col-span-3'>
    <svg aria-hidden="true" class="flex w-6 h-6 mr-2 text-gray-200 animate-spin dark:text-gray-600 fill-blue-600" viewBox="0 0 100 101" fill="none" xmlns="http://www.w3.org/2000/svg">
    ...
    {slots?.length===0 && selectedDate!=="" && responded && <p className='text-sm text-red-700'>No Available Slot</p>}
